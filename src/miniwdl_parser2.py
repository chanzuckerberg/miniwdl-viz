...
class MiniWDLParser2:
    ...
    def parse_input_item(self, input_item, node_ids):
        if isinstance(input_item, WDL.Expr.Ident):
            ...
            return [
                {
                    "name": f"{input_item.name}",
                    "ref": f"{ref}",
                    "type": "input"
                }
            ]
        elif isinstance(input_item, (
            WDL.Expr.String, WDL.Expr.Int, WDL.Expr.Float, WDL.Expr.Boolean, WDL.Expr.Null
        )):
            return [
                {
                    "name": f"{input_item}",
                    "ref": "HardcodedVariable",
                    "type": "input"
                }
            ]
            ...
            for child_input_item in input_item.children:
                names.extend(
                    self.parse_input_item(child_input_item, node_ids)
                )
            return names
        else:
            raise
    ...
    def add_node(self, nodes, parent, id, name, type):
        section = parent.name if isinstance(parent, WDL.Workflow) else parent.workflow_node_id
        nodes.append({
            "id": id,
            "name": name,
            "section": section,
    ...
    def trace_identifiers(self, wdl_doc, nodes, edges):
        if isinstance(wdl_doc, WDL.Workflow):
            ...
            for task_name, workflow_input in wdl_doc.inputs.items():
                input_param_list = self.parse_input_item(workflow_input, [node["id"] for node in nodes])
            ...
            if wdl_doc.expr:
                ...
                input_param_list = self.parse_input_item(wdl_doc.expr, [node["id"] for node in nodes])
                for input_param in input_param_list:
                    edges.append({
                        "node_from": input_param["ref"],
                        "node_to": wdl_doc.workflow_node_id,
                        "workflow_ref_name": input_param["name"],
                        "task_ref_name": input_param["name"],
                        "edge_type": "decl_to"
                    })
