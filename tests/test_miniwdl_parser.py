...
@pytest.fixture
def complex_wdl():
    doc = WDL.load(join(dirname(realpath(__file__)), "test_wdls", "joint-discovery-gatk4-version.wdl"))
    parser = MiniWDLParser2(doc)
    parser.parse()
    return parser
...
class TestMiniWDLParser:
    ...
    def test_complex_wdl(self, complex_wdl):
        ...
        subsection_nodes = [node for node in complex_wdl.nodes if node["type"] == "workflow_section"]
